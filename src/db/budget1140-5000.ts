import type { Budget } from "./budgets";

export const budget1440_5000: Budget = {
  id: "c7c0c8c1-6d52-4b82-9e2b-2e4f5f414c1c",
  buttonLabel: "1440P - 5000 Soles",
  title: "Procesador Amd Ryzen 5 7600",
  currency: "S/. ",
  peripheralsTitle: "Perifericos",
  peripherals: [],
  components: [
    {
      title: "Procesador Amd Ryzen 5 7600",
      price: 219.5,
      currency: "S/.",
      photoUrl:
        "https://www.impacto.com.pe/storage/products/lg/173836245884921.webp",
      urls: [
        {
          label: "Tienda Impacto",
          link: "https://www.impacto.com.pe/producto/procesador-amd-ryzen-5-7600-3-8ghz-hasta-5-1ghz-32mb-100-100001015box-am5-6-nucleos-c-cooler",
        },
        {
          label: "Mercado Libre",
          link: "https://www.mercadolibre.com.pe/procesador-amd-ryzen-7-9700x-am5-38ghz-55ghz/p/MPE39663823#polycard_client=search-nordic&search_layout=grid&position=1&type=product&tracking_id=692bd210-22ae-4315-a19d-0eeb5a2fa60e&wid=MPE816845660&sid=search",
        },
      ],
    },
    {
      title: "Tarjeta De Video Msi Geforce Rtx 5060 Ti 16gb",
      price: 554.0,
      currency: "S/.",
      photoUrl:
        "https://sercoplus.com/70262-large_default/vga-msi-geforce-rtx-5060-ti-16gb-shadow.jpg",

      urls: [
        {
          label: "Tienda Impacto",
          link: "https://www.impacto.com.pe/producto/tarjeta-de-video-msi-geforce-rtx-5060-ti-16gb-shadow-2x-oc-gddr7-90rtx5060ti-16-shadow-2xo-nvidia-128-bits-2-ventiladores",
        },
        {
          label: "Mercado Libre",
          link: "https://www.mercadolibre.com.pe/tarjeta-de-video-nvidia-gigabyte-geforce-rtx-5070-gv-n5070wf30c-12gd-windforce-oc-sff-12gb/p/MPE49431202#polycard_client=search-nordic&search_layout=grid&position=6&type=product&tracking_id=2fc19b22-7ece-462d-8d68-f3769d5e9793&wid=MPE727288210&sid=search",
        },
      ],
    },
    {
      title: "Placa Madre Asus B850m-plus Tuf Gaming Wifi",
      price: 252.0,
      currency: "S/.",
      photoUrl:
        "https://www.impacto.com.pe/storage/products/md/174829114840336.webp",
      urls: [
        {
          label: "Tienda Impacto",
          link: "https://www.impacto.com.pe/producto/placa-madre-asus-b850m-plus-tuf-gaming-wifi-90mb1iyo-m0eayo-socket-am5-ddr5",
        },
        {
          label: "Mercado Libre",
          link: "https://www.mercadolibre.com.pe/motherboard-asrock-x870-pro-rs-wifi-socket-amd-am5-atx/up/MPEU2925714068#polycard_client=search-nordic&search_layout=grid&position=1&type=product&tracking_id=5002b591-5ddb-4716-9873-5cf96db46a24&wid=MPE696825956&sid=search",
        },
      ],
    },
    {
      title: "Case Gambyte Shield S/fuente",
      price: 37.5,
      currency: "S/.",
      photoUrl:
        "https://www.impacto.com.pe/storage/products/lg/173888385078351.webp",
      urls: [
        {
          label: "Tienda Impacto",
          link: "https://www.impacto.com.pe/producto/case-gambyte-shield-sfuente-cs-shi4rbwkg-lg-paneles-de-vidrio-5-ventiladores-led-rainbow-negro",
        },
        {
          label: "Mercado Libre",
          link: "https://www.mercadolibre.com.pe/gabinete-gamer-deepcool-ch780-blanco-gaming/p/MPE36809528#polycard_client=search-nordic&search_layout=grid&position=2&type=product&tracking_id=3318c884-b4d4-482c-8e3e-89914db0f84d&wid=MPE723206720&sid=search",
        },
      ],
    },
    {
      title: "Memoria Ram Corsair Ddr5 16gb Vengeance Rgb x2",
      price: 169.0,
      currency: "S/.",
      photoUrl:
        "https://www.impacto.com.pe/storage/products/lg/175166210438977.webp",
      urls: [
        {
          label: "Tienda Impacto",
          link: "https://www.impacto.com.pe/producto/memoria-ram-corsair-ddr5-16gb-vengeance-rgb-cmh16gx5m1b5600z40-5600mts-rgb-doble-perfil-amd-e-intel",
        },
        {
          label: "Mercado Libre",
          link: "https://www.mercadolibre.com.pe/memoria-rgb-corsair-vengeance-de-16-gb-ddr5-5600-mhz-para-ordenador/p/MPE40654040#polycard_client=search-nordic&search_layout=grid&position=2&type=product&tracking_id=055c9af1-96ec-4876-b7b1-8c9b04eab4ef&wid=MPE757435714&sid=search",
        },
      ],
    },
    {
      title: "Cooler Para Procesador Deepcool Ak500 Zero Dark",
      price: 68.0,
      currency: "S/.",
      photoUrl:
        "https://www.impacto.com.pe/storage/products/md/173886285434392.webp",
      urls: [
        {
          label: "Tienda Impacto",
          link: "https://www.impacto.com.pe/producto/cooler-para-procesador-deepcool-ak500-zero-dark-r-ak500-bknnmt-g-1-1-ventilador-120mm",
        },
        {
          label: "Mercado Libre",
          link: "https://www.mercadolibre.com.pe/refrigeracion-liquida-deepcool-mystique-360-argb-pantalla/up/MPEU3132518130#polycard_client=search-nordic&search_layout=grid&position=3&type=product&tracking_id=6c2da189-3b63-4da5-a71c-2aa9aa7a1f34&wid=MPE712143520&sid=search",
        },
      ],
    },
    {
      title: "Unidad De Almacenamiento Ssd M.2 Pcie Kingston 1tb Nvme Nv3",
      price: 85.5,
      currency: "S/.",
      photoUrl:
        "https://www.impacto.com.pe/storage/products/lg/173835116992953.webp",
      urls: [
        {
          label: "Tienda Impacto",
          link: "https://www.impacto.com.pe/producto/unidad-de-almacenamiento-ssd-m-2-pcie-kingston-1tb-nvme-nv3-2280-snv3s1000g-6000mbs",
        },
        {
          label: "Mercado Libre",
          link: "https://www.mercadolibre.com.pe/unidad-ssd-m2-pcie-1tb-wd-black-sn8100-g5/up/MPEU3368728635#polycard_client=search-nordic&search_layout=grid&position=5&type=product&tracking_id=50f0e3dc-3567-4dba-b510-744dce9a4234&wid=MPE806978636&sid=search",
        },
      ],
    },
    {
      title: "Fuente De Poder Corsair 850w Rm850e",
      price: 124.0,
      currency: "S/.",
      photoUrl:
        "https://www.impacto.com.pe/storage/products/md/174343471663246.webp",
      urls: [
        {
          label: "Tienda Impacto",
          link: "https://www.impacto.com.pe/producto/fuente-de-poder-corsair-850w-rm850e-cp-9020296-na-atx-80-plus-gold-modular-negro-pcie-5",
        },
        {
          label: "Mercado Libre",
          link: "https://www.mercadolibre.com.pe/fuente-de-poder-asus-prime-750w-80-gold-white-modular-atx/up/MPEU3433571846#polycard_client=search-nordic&search_layout=grid&position=8&type=product&tracking_id=fb207f19-0eeb-4ed1-b7b3-b6820b543164&wid=MPE438966480&sid=search",
        },
      ],
    },
  ],
};
